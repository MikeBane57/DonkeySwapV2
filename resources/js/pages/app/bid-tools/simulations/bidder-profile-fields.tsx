...
export function emptyBidderProfile(defaults: BidderProfile): BidderProfile {
    return {
        ...
        weights: { ...defaults.weights, criteria_order: [...defaults.weights.criteria_order] },
        personal_dates: [],
        vacation_ranges: [],
    };
}
        ...
        <div className="space-y-4 rounded-md border border-sidebar-border/50 bg-muted/20 p-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
                ...
                <WeightSlider
                    ...
                    onChange={(vacation_penalty) => setWeights({ vacation_penalty })}
                />
            </div>
            ...
            <div className="space-y-2">
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                    {value.weights.criteria_order.map((key, idx) => (
                        <div key={`${idPrefix}-crit-${idx}`} className="flex items-center gap-2 text-xs">
                            <span className="w-6 text-muted-foreground">{idx + 1}.</span>
                            <Select
                                value={key}
                                onValueChange={(next) => {
                                    const order = [...value.weights.criteria_order];
