                    ...
                    >
                        <div className="space-y-3">
                            <Label className="text-sm font-medium">
                                Shift start time range (optional)
                            </Label>
                            <p className="text-xs text-muted-foreground">
                                Only show shifts that start within this window.
                                Leave blank for no limit. Times in your profile
                                timezone.
                            </p>
                            ...
                            {(errors.shift_start_time_min ||
                                errors.shift_start_time_max) && (
                        ...
                        <div className="space-y-4">
                            <div>
                                <Label className="text-sm font-medium">
                                    Show me shifts for these desks
                                </Label>
                                <p className="mt-0.5 text-xs text-muted-foreground">
                                    Check the desk types you want to see on the
                                    Available page. Leave all unchecked to see
                                    all desks you&apos;re qualified for. Grouped
                                    by workgroup.
                                </p>
                            </div>
                            {desk_options_by_workgroup.length === 0 ? (
                                <p className="text-sm text-muted-foreground">
                                    No desk types available for your workgroups
                                    yet.
                                </p>
                            ) : (
                                <div className="space-y-5">
                                    {desk_options_by_workgroup.map(
                                        (section) => (
                                            <div
                                                key={section.workgroup_id}
                                                className="rounded-lg border border-sidebar-border/70 bg-muted/10 p-4 dark:border-sidebar-border"
                                            >
                                                <h3 className="mb-3 text-sm font-semibold text-foreground">
                                                    {section.workgroup_name}
                                                </h3>
                                                <div className="flex flex-wrap gap-4">
                                                    {section.desk_type_options.map(
                                                        (opt) => (
                                                            <Label
                                                                key={opt.value}
                                                            ...
                                                            >
                                                                <Checkbox
                                                                    checked={data.desired_desk_types.includes(
                                                                        opt.value,
                                                                    )}
                                                                    onCheckedChange={() =>
                                                                        toggleDeskType(
                        ...
                        <div className="space-y-4 rounded-lg border border-sidebar-border/70 bg-muted/20 p-4 dark:border-sidebar-border">
                            <div>
                                <Label className="text-sm font-medium">
                                    Non-regulatory workgroups — doubles
                                </Label>
                                <p className="mt-0.5 text-xs text-muted-foreground">
                                    If you&apos;re willing to work doubles,
                                    which combinations and what gap between
                                    shifts is acceptable.
                                </p>
                            </div>
                            <div className="flex flex-wrap gap-6">
                                <Label className="flex cursor-pointer items-center gap-2 font-normal">
                                    <Checkbox
                                        ...
                                        onCheckedChange={(c) =>
                                            setData('willing_double_am_pm', !!c)
                                        }
                                    />
                                ...
                                <Label className="flex cursor-pointer items-center gap-2 font-normal">
                                    <Checkbox
                                        checked={
                                            data.willing_double_pm_midnight
                                        }
                                        onCheckedChange={(c) =>
                                            setData(
                                                'willing_double_pm_midnight',
                                ...
                                <Label className="flex cursor-pointer items-center gap-2 font-normal">
                                    <Checkbox
                                        checked={
                                            data.willing_double_midnight_am
                                        }
                                        onCheckedChange={(c) =>
                                            setData(
                                                'willing_double_midnight_am',
                            ...
                            <div className="flex flex-wrap items-end gap-4">
                                <div>
                                    <Label className="text-xs">
                                        Max gap between shifts (minutes)
                                    </Label>
                                    <Input
                                        type="number"
                                        min={0}
                                        ...
                                        value={
                                            data.double_gap_minutes_acceptable ===
                                            ''
                                                ? ''
                                                : data.double_gap_minutes_acceptable
                                        }
                                    ...
                                    />
                                    <p className="mt-0.5 text-xs text-muted-foreground">
                                        e.g. 60 = 1 hour between end of first
                                        and start of next
                                    </p>
                                </div>
                                <div>
                                    <Label className="text-xs">
                                        Max doubles in a row
                                    </Label>
                                    <Input
                                        type="number"
                                        min={1}
                            ...
                            {(errors.double_gap_minutes_acceptable ||
                                errors.max_doubles_in_row) && (
                                <p className="text-sm text-destructive">
                                    {errors.double_gap_minutes_acceptable ??
                                        errors.max_doubles_in_row}
                                </p>
                            ...
                            <div className="border-t border-sidebar-border/50 pt-2">
                                <Label className="flex cursor-pointer items-center gap-2 font-normal">
                                    <Checkbox
                                        checked={
                                            data.hide_posts_that_would_be_double
                                        }
                                        onCheckedChange={(c) =>
                                            setData(
                                                'hide_posts_that_would_be_double',
                                    ...
                                    />
                                    <span className="text-sm">
                                        Hide posts that would be a double on the
                                        Available page
                                    </span>
                                </Label>
                                <p className="mt-1 ml-6 text-xs text-muted-foreground">
                                    When on, giveaways or trades that would give
                                    you two shifts on the same day are hidden.
                                    Turn off to see them (they will show a
                                    &quot;Double&quot; badge).
                                </p>
                            </div>
                        </div>
