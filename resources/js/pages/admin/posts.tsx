...
export default function AdminPosts() {
    ...
    const { posts, users, workgroups, status_options = [], filters = {}, sort, dir, flash } = props;
    const statusOptions = status_options.length > 0 ? status_options : [
    ...
    const [filterWorkgroup, setFilterWorkgroup] = useState(filters.workgroup_id ?? ALL_VALUE);
    const [filterStatus, setFilterStatus] = useState(filters.status ?? ALL_VALUE);
    ...
    const [filterMinTransactions, setFilterMinTransactions] = useState(filters.min_transactions ?? '');
    const [filterMinOffers, setFilterMinOffers] = useState(filters.min_offers ?? '');
    const [filterSearch, setFilterSearch] = useState(filters.search ?? '');
    const [sortField, setSortField] = useState(sort ?? 'posts_created_at');
    const [sortDir, setSortDir] = useState(dir ?? 'desc');

    const applyFilters = () => {
        const params: Record<string, string> = { sort: sortField, dir: sortDir };
        if (filterUser && filterUser !== ALL_VALUE) params.user_id = filterUser;
        if (filterWorkgroup && filterWorkgroup !== ALL_VALUE) params.workgroup_id = filterWorkgroup;
        if (filterStatus && filterStatus !== ALL_VALUE) params.status = filterStatus;
        if (filterType && filterType !== ALL_VALUE) params.type = filterType;
        if (filterDateFrom) params.date_from = filterDateFrom;
        if (filterDateTo) params.date_to = filterDateTo;
        if (filterMinTransactions.trim() !== '') params.min_transactions = filterMinTransactions.trim();
        if (filterMinOffers.trim() !== '') params.min_offers = filterMinOffers.trim();
    ...
    const setShiftPostsStatus = (shiftId: number, status: 'open' | 'closed' | 'cancelled') => {
        router.put(`/app/admin/shifts/${shiftId}/posts-status`, { status }, { preserveScroll: true });
    };

    const removeShiftPosts = (shiftId: number) => {
        if (confirm('Remove all posts for this shift? This cannot be undone.')) {
            router.delete(`/app/admin/shifts/${shiftId}/posts`, { preserveScroll: true });
        }
    };
    ...
    const toggleSort = (field: string) => {
        ...
        router.get('/app/admin/posts', {
            ...
            workgroup_id: filterWorkgroup !== ALL_VALUE ? filterWorkgroup : undefined,
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="p-4 space-y-4">
                ...
                {flash?.success && (
                    <p className="text-sm text-green-600 dark:text-green-400">{flash.success}</p>
                )}
                {flash?.error && (
                    <p className="text-sm text-red-600 dark:text-red-400">{flash.error}</p>
                )}

                {/* Filters */}
                <div className="rounded-lg border border-sidebar-border/70 p-4 space-y-4 dark:border-sidebar-border">
                    <div className="space-y-2">
                        <Label>Search (owner name/email or shift position)</Label>
                        <Input
                            placeholder="Search..."
                            value={filterSearch}
                    ...
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <Select value={filterWorkgroup} onValueChange={setFilterWorkgroup}>
                                <SelectTrigger>
                                    <SelectValue placeholder="All workgroups" />
                                </SelectTrigger>
                                <SelectContent>
                                    <SelectItem value={ALL_VALUE}>All workgroups</SelectItem>
                                    {workgroups.map((wg) => (
                        ...
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                onChange={(e) => setFilterMinTransactions(e.target.value)}
                            />
                        </div>
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                onChange={(e) => setFilterMinOffers(e.target.value)}
                            />
                        </div>
                        <div className="space-y-2">
                            <Label>Sort</Label>
                                    ...
                                    <Fragment key={rowKey}>
                                            <TableRow className={expandedShiftId === rowKey ? 'bg-muted/50' : ''}>
                                                ...
                                                <TableCell className="whitespace-nowrap text-muted-foreground">
                                                    {row.posts_created_at ? formatDate(row.posts_created_at) : '—'}
                                                </TableCell>
                                                <TableCell>
                                                ...
                                                <TableCell>
                                                    {row.shift ? (
                                                        <>
                                                            <div>{row.shift.position_name}</div>
                                                ...
                                                <TableCell>{row.types_label || '—'}</TableCell>
                                                <TableCell className="text-right">
                                                    {row.cash_amount != null && Number(row.cash_amount) > 0
                                                        ? `$${Number(row.cash_amount).toFixed(2)}`
                                                        : '—'}
                                                </TableCell>
                                                <TableCell>
                                                    <span className={
                                                        row.status === 'Open' ? 'text-green-600 dark:text-green-400' :
                                                        row.status === 'Accepted' ? 'text-blue-600 dark:text-blue-400' :
                                                ...
                                                <TableCell>
                                                    {row.shift?.assignee_name ? (
                                                        <>
                                                            <div>{row.shift.assignee_name}</div>
                                                            <div className="text-xs text-muted-foreground">{row.shift.assignee_email ?? ''}</div>
                                                        </>
                                                    ) : (
                                                        '—'
                                                    )}
                                                </TableCell>
                                                <TableCell className="text-right">{row.view_count}</TableCell>
                                                <TableCell className="text-right">{row.click_count}</TableCell>
                                                <TableCell className="text-right" title="Distinct users who have hidden this post">
                                                    {row.hidden_by_count ?? 0}
                                                </TableCell>
                                                <TableCell className="text-right" title="Times this shift has traded hands">
                                                ...
                                                <TableRow className="bg-muted/30 hover:bg-muted/30">
                                                    <TableCell colSpan={15} className="p-4">
                                                        <div className="grid gap-6 md:grid-cols-2">
                                                            <div>
                                                                ...
                                                                {(!row.activity || row.activity.length === 0) ? (
                                                                    ...
                                                                    <ul className="space-y-2 text-sm">
                                                                        {row.activity.map((item, idx) => (
                                                                            <li key={idx} className="rounded border border-sidebar-border/70 p-2 dark:border-sidebar-border">
                                                                                <span className="text-muted-foreground">{item.at ? formatDateTime(item.at) : '—'}</span>
                                                                                <div className="font-medium mt-0.5">{item.label}</div>
                                                                                {item.actor && <div className="text-xs text-muted-foreground">by {item.actor}</div>}
                                                                                {item.event === 'edit' && item.changes != null && (
                                                                                    <pre className="mt-1 whitespace-pre-wrap break-words text-xs">
                                                                                        {typeof item.changes === 'object' ? JSON.stringify(item.changes, null, 2) : String(item.changes)}
                                                                                    </pre>
                                                                                )}
                                                                            </li>
                                                                        ))}
                                                            ...
                                                            <div>
                                                                <h4 className="font-medium mb-2">Offers & responses</h4>
                                                                {(!row.offers || row.offers.length === 0) ? (
                                                                    <p className="text-sm text-muted-foreground">No offers.</p>
                                                                ) : (
                                                                    <ul className="space-y-2 text-sm">
                                                                        {row.offers.map((o) => (
                                                                            <li key={o.id} className="rounded border border-sidebar-border/70 p-3 dark:border-sidebar-border">
                                                                                <div className="flex flex-wrap items-center justify-between gap-2">
                                                                                    <span className="font-medium">{o.offered_by_name ?? o.offered_by_email ?? 'Unknown'}</span>
                                                                                    <span className={
                                                                                        o.status === 'accepted' ? 'text-green-600 dark:text-green-400' :
                                                                                        o.status === 'rejected' ? 'text-red-600 dark:text-red-400' :
                                                                                        'text-muted-foreground'
                                                                                    }>
                                                                                        {o.status}
                                                                                    </span>
                                                                                </div>
                                                                                {(o.poster_name ?? o.poster_email) && (
                                                                                    <div className="text-muted-foreground text-xs mt-1">Posted by: {o.poster_name ?? o.poster_email ?? '—'}</div>
                                                                                )}
                                                                                {o.shift_going_to_name && (
                                                                                    <div className="text-muted-foreground text-xs mt-0.5">Shift going to: {o.shift_going_to_name}</div>
                                                                                )}
                                                                                {o.cash_amount != null && Number(o.cash_amount) > 0 && (
                                                                                    <div className="text-xs mt-0.5 font-medium text-foreground">${Number(o.cash_amount).toFixed(2)}</div>
                                                                                )}
                                                                                {o.offered_shift_summary && (
                                                                                    <div className="text-muted-foreground text-xs mt-0.5">{o.offered_shift_summary}</div>
                                                                                )}
                                                                                {o.response_notes && (
                                                                                    <div className="mt-1 text-xs border-t border-sidebar-border/70 pt-1 dark:border-sidebar-border">
                                                                                        Response: {o.response_notes}
                                                                                    </div>
                                                                                )}
                                                                                <div className="text-muted-foreground text-xs mt-0.5">{formatDateTime(o.created_at)}</div>
                                                                            </li>
                                                                        ))}
                                                                    </ul>
                                                                )}
                    ...
                    <div className="flex items-center justify-center gap-2 flex-wrap">
                        {posts.links.map((link, i) => (
                            <span key={i}>
                                    ...
                                    <a
                                        href={link.url}
                                        className={`px-2 py-1 rounded text-sm ${link.active ? 'bg-primary text-primary-foreground' : 'hover:underline'}`}
                                        onClick={(e) => {
                                            e.preventDefault();
                                    ...
                                    <span className="px-2 py-1 text-muted-foreground text-sm">{decodePaginationLabel(link.label)}</span>
                                )}
                            </span>
                        ))}
                    </div>
