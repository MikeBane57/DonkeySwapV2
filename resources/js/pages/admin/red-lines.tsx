...
export default function AdminRedLines() {
    ...
    const [aboveLineSeniority, setAboveLineSeniority] = useState<
        { user_id: number; seniority_number: number | null }[]
    >([]);
    ...
    const submitCreate = (e: React.FormEvent) => {
        e.preventDefault();
        const wgId = createForm.workgroup_id
            ? parseInt(createForm.workgroup_id, 10)
            : null;
        if (!wgId) return;
        router.post(
            '/app/admin/red-lines',
    ...
    const setSeniorityForUser = (userId: number, value: number | null) => {
        setAboveLineSeniority((prev) =>
            prev.map((u) =>
                u.user_id === userId ? { ...u, seniority_number: value } : u,
            ),
        );
    };
    ...
    const submitUpdate = (e: React.FormEvent) => {
        ...
        if (!editing) return;
        router.put(
            `/app/admin/red-lines/${editing.id}`,
            {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">
                            Set red line position per workgroup. Users are
                            ordered by classification seniority (oldest first =
                            above line).
                        </p>
                    </div>
                    <Button
                    ...
                    {redLines.length === 0 && !createOpen && (
                        <div className="rounded-xl border border-sidebar-border/70 p-6 dark:border-sidebar-border">
                            <p className="text-sm text-muted-foreground">
                                No red lines defined. Create a workgroup first,
                                then add a red line for it.
                            </p>
                        </div>
                    )}
                    {redLines.map((rl) => (
                        ...
                        >
                            <div className="flex items-center justify-between border-b border-sidebar-border/70 bg-muted/50 px-4 py-2">
                                <span className="font-medium">
                                    {rl.workgroup_name ??
                                        `Workgroup #${rl.workgroup_id}`}{' '}
                                    — Red line at position{' '}
                                    {rl.red_line_position}
                                </span>
                            ...
                            <div className="grid grid-cols-1 gap-4 p-4 md:grid-cols-2">
                                <div>
                                    <h3 className="text-sm font-medium text-green-700 dark:text-green-400">
                                        Above line ({rl.users_above.length})
                                    </h3>
                                    <ul className="mt-1 space-y-1 text-sm">
                                        {rl.users_above.length === 0 && (
                                ...
                                <div>
                                    <h3 className="text-sm font-medium text-amber-700 dark:text-amber-400">
                                        Below line ({rl.users_below.length})
                                    </h3>
                                    <ul className="mt-1 space-y-1 text-sm">
                                        {rl.users_below.length === 0 && (
                                            <li className="text-muted-foreground">
                                                None
                                            </li>
                                        )}
                    ...
                    <form onSubmit={submitCreate} className="space-y-4">
                        ...
                        <div>
                            <Label htmlFor="create-position">
                                Red line position (number of users above the
                                line)
                            </Label>
                            <Input
                                id="create-position"
                    ...
                    <form onSubmit={submitUpdate} className="space-y-4">
                        <div>
                            <Label htmlFor="edit-position">
                                Position (number of users above the line)
                            </Label>
                            <Input
                                id="edit-position"
                                type="number"
                        ...
                        {editing && editing.users_above.length > 0 && (
                            <div>
                                <Label className="mb-2 block">
                                    Seniority number (above the line)
                                </Label>
                                <p className="mb-2 text-xs text-muted-foreground">
                                    Assign a seniority number (1 = highest) for
                                    each person above the line. Lower number =
                                    higher seniority. Leave blank to use
                                    classification seniority date order.
                                </p>
                                <div className="space-y-2 rounded-md border p-3">
                                    {aboveLineSeniority.map((u) => {
                                        const user = editing.users_above.find(
                                            (x) => x.id === u.user_id,
                                        );
                                        return (
                                            <div
            ...
            >
                <DialogContent>
                    ...
                    <p className="text-sm text-muted-foreground">
                        This will remove the red line for this workgroup. It can
                        be recreated later.
                    </p>
                    <DialogFooter>
                        <Button
