...
type DeskType = {
    ...
    workgroup_qualification_code?: string;
};
type PositionRange = {
    id?: number;
    range_spec: string;
    parity: string;
    desk_type_code: string;
};
...
const emptyQualification = (): { code: string; label: string } => ({
    code: '',
    label: '',
});
...
function formatPositionRange(r: PositionRange, deskTypes?: DeskType[]): string {
    const label =
        deskTypes?.find((d) => d.code === r.desk_type_code)?.label ??
        DESK_TYPE_CODE_LABELS[r.desk_type_code] ??
        r.desk_type_code;
...
export default function AdminWorkgroups() {
            ...
            desk_types:
                (wg.desk_types ?? []).length > 0
                    ? wg.desk_types.map((d) => ({
                          ...
                          label: d.label,
                          workgroup_qualification_code:
                              d.workgroup_qualification_code ?? '',
                      }))
                    : [],
            position_ranges:
    ...
    const submitSectionUpdate = (e: React.FormEvent) => {
        ...
        const payload = {
            ...form,
            desk_types: form.desk_types.filter(
                (d) => d.code.trim() !== '' && d.label.trim() !== '',
            ),
            position_ranges: form.position_ranges.filter(
                (r) => r.range_spec.trim() !== '',
            ),
            qualifications: form.qualifications.filter(
                (q) => q.code.trim() !== '' && q.label.trim() !== '',
            ),
        };
    ...
    const removeStartTime = (index: number) => {
        setForm((f) => ({
            ...f,
            allowed_start_times: f.allowed_start_times.filter(
                (_, i) => i !== index,
            ),
        }));
    };
    ...
    const updateStartTime = (
        index: number,
        field: 'start_time' | 'default_duration_minutes',
        value: string | number,
    ) => {
        setForm((f) => ({
            ...f,
    ...
    const addPositionRange = () => {
        setForm((f) => ({
            ...f,
            position_ranges: [...f.position_ranges, emptyPositionRange()],
        }));
    };
    const removePositionRange = (index: number) => {
        setForm((f) => ({
            ...f,
            position_ranges: f.position_ranges.filter((_, i) => i !== index),
        }));
    };
    const updatePositionRange = (
        index: number,
        field: 'range_spec' | 'parity' | 'desk_type_code',
        value: string,
    ) => {
        setForm((f) => ({
            ...f,
            position_ranges: f.position_ranges.map((r, i) =>
    ...
    const addDeskType = () => {
        setForm((f) => ({
            ...f,
            desk_types: [...f.desk_types, emptyDeskType()],
        }));
    };
    const removeDeskType = (index: number) => {
        setForm((f) => ({
            ...f,
            desk_types: f.desk_types.filter((_, i) => i !== index),
        }));
    };
    const updateDeskType = (
        index: number,
        field:
            | 'code'
            | 'label'
        ...
        setForm((f) => ({
            ...f,
            desk_types: f.desk_types.map((d, i) =>
    ...
    const deskTypeOptions = (): { value: string; label: string }[] => {
        if (form.desk_types.length === 0)
            return [{ value: '', label: '(Add desk types first)' }];
        return form.desk_types
    ...
    const addQualification = () => {
        setForm((f) => ({
            ...f,
            qualifications: [...f.qualifications, emptyQualification()],
        }));
    };
    ...
    const updateQualification = (
        ...
        setForm((f) => ({
            ...f,
            qualifications: f.qualifications.map((q, i) =>
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">
                            Create workgroups, set max hours, rest hours,
                            allowed start times, and desk types (regulatory per
                            desk type).
                        </p>
                    </div>
                        ...
                        <table className="w-full min-w-max text-sm">
                            <thead>
                                <tr className="border-b border-sidebar-border/70 bg-muted/50">
                                    ...
                                    <th className="min-w-[10rem] p-3 text-left font-medium">
                                        Desk types
                                    </th>
                                    <th className="min-w-[12rem] p-3 text-left font-medium">
                                        Position ranges
                                    </th>
                                    <th className="min-w-[10rem] p-3 text-left font-medium">
                                        Qualifications
                            ...
                            <tbody>
                                {workgroups.length === 0 && (
                                    <tr>
                                        ...
                                        >
                                            No workgroups yet. Click “Add
                                            workgroup” to create one.
                                        </td>
                                    </tr>
                                )}
                                {workgroups.map((wg) => (
                                    <tr
                                        key={wg.id}
                                        className="border-b border-sidebar-border/50 align-top"
                                    >
                                        <td className="p-3">
                                        ...
                                        <td className="p-3">
                                            {wg.max_hours_per_day ?? '—'}
                                        </td>
                                        <td className="p-3">
                                            {wg.rest_required_hours ?? '—'}
                                        </td>
                                        <td className="p-3">
                                            {wg.allow_double ? 'Yes' : 'No'}
                                        </td>
                                        <td className="p-3">
                                            <div className="flex flex-wrap items-center gap-1">
                                                {wg.allowed_start_times
                                                    .length === 0 ? (
                                                    <span className="text-muted-foreground">
                                                        —
                                                    </span>
                                                ) : (
                                                    wg.allowed_start_times.map(
                                                        (t) => (
                                                            <span
                                                                key={t.id}
                        ...
                        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-6 py-2">
                            ...
                            <div>
                                ...
                                <p className="mt-1 text-xs text-muted-foreground">
                                    Define desk types for this workgroup (e.g.
                                    Domestic dispatch, Assistant desk,
                                    Regional). Optionally require a
                                    qualification to work each type.
                                </p>
                                <div className="mt-2 space-y-2">
                            ...
                            <div>
                                <div className="flex items-center justify-between">
                                    ...
                                    <Button
                                        ...
                                        variant="outline"
                                        size="sm"
                                        onClick={addPositionRange}
                                    >
                                        Add range
                                    </Button>
                                </div>
                                <p className="mt-1 text-xs text-muted-foreground">
                                    Range: e.g. 1-98, G1-G4. Assign each range
                                    to a desk type above.
                                </p>
                                <div className="mt-2 space-y-2">
                            ...
                            <div>
                                <div className="flex items-center justify-between">
                                    <Label>
                                        Qualifications (dispatcher / worker)
                                    </Label>
                                    <Button
                                        type="button"
                                        variant="outline"
                                ...
                                <p className="mt-1 text-xs text-muted-foreground">
                                    e.g. Dispatch: ETOPS, INTL, ASST. Users can
                                    be assigned these per workgroup. Default new
                                    users: Dispatch + ASST.
                                </p>
                                <div className="mt-2 space-y-2">
            ...
            <Dialog
                open={!!editSection && !!editing}
                onOpenChange={(open) => !open && closeSectionEdit()}
            >
                        ...
                        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-6 py-2">
                            ...
                            {editSection === 'desk_types' && (
                                <div>
                                    ...
                                    <p className="mt-1 text-xs text-muted-foreground">
                                        Code (e.g. domestic_dispatch), label,
                                        regulatory flag, and optional required
                                        qualification.
                                    </p>
                                    <div className="mt-2 space-y-2">
                                        {form.desk_types.map((d, i) => (
                            ...
                            {editSection === 'position_ranges' && (
                                <div>
                                    ...
                                    <p className="mt-1 text-xs text-muted-foreground">
                                        Range: e.g. 1-98, G1-G4. Assign each to
                                        a desk type.
                                    </p>
                                    <div className="mt-2 space-y-2">
                                        {form.position_ranges.map((r, i) => (
            ...
            >
                <DialogContent>
                    ...
                    <p className="text-sm text-muted-foreground">
                        This will remove the workgroup and its allowed start
                        times. Users assigned to this workgroup will be
