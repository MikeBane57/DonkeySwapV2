...
export function PostShiftModal({
    ...
    const [tradeCash, setTradeCash] = useState(existingTrade?.cash_amount?.toString() ?? '');
    const [cashAmount, setCashAmount] = useState(existingCash?.cash_amount?.toString() ?? '');
    const [ffCash, setFfCash] = useState(existingFF?.cash_amount?.toString() ?? '');
    const [ffMinutes, setFfMinutes] = useState(existingFF?.flight_follow_minutes?.toString() ?? '90');
    const [ffAt, setFfAt] = useState<'beginning' | 'end'>(
        (existingFF?.flight_follow_at === 'end' ? 'end' : 'beginning')
    );
    const [timeTradeCash, setTimeTradeCash] = useState(existingTimeTrade?.cash_amount?.toString() ?? '');
    ...
    const hasExisting = !!existingTrade || !!existingCash || !!existingFF || !!existingTimeTrade;
    ...
    useEffect(() => {
        if (!open) return;
        if (existingTimeTrade?.preferred_start_times && Array.isArray(existingTimeTrade.preferred_start_times)) {
            setPreferredStartTimes([...existingTimeTrade.preferred_start_times]);
        } else {
    ...
    const handleRemovePost = async () => {
        ...
        try {
            const res = await fetch(`/api/shifts/${shift.shiftId}/postings`, {
                ...
                credentials: 'include',
                body: JSON.stringify({ postings: [], delete_ids: safePosts.map((p) => p.id) }),
            });
            if (res.ok) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (postAsFF) {
            postings.push({
                ...
                flight_follow_minutes: ffMinutes ? parseInt(ffMinutes, 10) : undefined,
        ...
        const deleteIds = [
            ...
            ...(!postAsTimeTrade && existingTimeTrade ? [existingTimeTrade.id] : []),
    ...
    return (
                ...
                <form onSubmit={handleSubmit}>
                    <div className="space-y-4 py-2">
                        <div className="rounded-xl border bg-muted/30 p-3">
                            ...
                            <div className="font-semibold">{shift.position_name}</div>
                            {shift.desk_type && (
                                <div className="text-xs text-muted-foreground">
                                    Desk type: {shift.workgroup_id != null
                                        ? (workgroups.find((w) => w.id === shift.workgroup_id)?.desk_types?.find((d) => d.code === shift.desk_type)?.label ?? DESK_TYPE_LABELS[shift.desk_type] ?? shift.desk_type)
                                        : (DESK_TYPE_LABELS[shift.desk_type] ?? shift.desk_type)}
                                </div>
                            )}
                            <div className="text-sm text-muted-foreground">
                                {timeStr}
                                {shift.workgroup_name ? ` · ${shift.workgroup_name}` : ''}
                            </div>
                        </div>
                        ...
                        {postAsFF && (
                            <div className="flex items-start gap-2 rounded-lg border border-amber-500/40 bg-amber-500/5 px-3 py-2 text-xs text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200">
                                ...
                                <p>
                                    Flight following cannot be combined with trade, time trade, or giveaway for this shift.
                                    {(existingTrade || existingCash || existingTimeTrade) && (
                                        <> Selecting flight following will deactivate your current post for this shift.</>
                                    )}
                                </p>
                            </div>
                        )}
                        {(postAsTrade || postAsCash || postAsTimeTrade) && existingFF && (
                            <div className="flex items-start gap-2 rounded-lg border border-amber-500/40 bg-amber-500/5 px-3 py-2 text-xs text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200">
                                <Info className="size-4 shrink-0 mt-0.5" />
                                <p>
                                    Trade, time trade, and giveaway cannot be combined with flight following. Selecting one will deactivate your current flight following post.
                                </p>
                            </div>
                        )}
                        {postAsTimeTrade && (
                            <div className="flex items-start gap-2 rounded-lg border border-amber-500/40 bg-amber-500/5 px-3 py-2 text-xs text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200">
                                <Info className="size-4 shrink-0 mt-0.5" />
                                <p>
                                    Time trade cannot be combined with trade, giveaway, or flight following for this shift.
                                    {(existingTrade || existingCash || existingFF) && (
                                        <> Selecting time trade will deactivate your current post for this shift.</>
                                    )}
                                </p>
                            </div>
                        )}
                        {(postAsTrade || postAsCash || postAsFF) && existingTimeTrade && (
                            <div className="flex items-start gap-2 rounded-lg border border-amber-500/40 bg-amber-500/5 px-3 py-2 text-xs text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200">
                                <Info className="size-4 shrink-0 mt-0.5" />
                                <p>
                                    Trade, giveaway, and flight following cannot be combined with time trade. Selecting one will deactivate your current time trade post.
                                </p>
                            </div>
                        )}
                        ...
                        <div className="space-y-3">
                            ...
                            >
                                <input
                                    ...
                                    className={cn(
                                        ...
                                        postAsTrade && 'border-primary bg-primary text-primary-foreground'
                                    )}
                            ...
                            >
                                <input
                                    ...
                                    className={cn(
                                        'size-4 shrink-0 rounded-[4px] border border-input shadow-xs',
                                        'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2',
                                        postAsCash && 'border-primary bg-primary text-primary-foreground'
                                    )}
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-xs text-muted-foreground">Give away shift, no trade required</p>
                                    {postAsCash && (
                                        <div className="mt-2">
                                            <Label className="text-xs">Cash (optional)</Label>
                                            <div className="relative mt-0.5">
                            ...
                            <div className="space-y-3 pt-1 border-t border-border pt-3 mt-1">
                                ...
                                >
                                    <input
                                        ...
                                        className={cn(
                                            'size-4 shrink-0 rounded-[4px] border border-input shadow-xs',
                                            'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2',
                                            postAsTimeTrade && 'border-primary bg-primary text-primary-foreground'
                                        )}
                                        aria-label="Post as time trade"
                                    />
                                    <div className="flex-1">
                                        <Label className="flex cursor-pointer items-center gap-2 font-medium">
                                            <Repeat strokeWidth={2.5} className="h-4 w-4 text-blue-600 dark:text-blue-400" />
                                            Time trade
                                        </Label>
                                        <p className="text-xs text-muted-foreground">Trade to a more preferable time (e.g. AM to PM, or later start same day)</p>
                                        {postAsTimeTrade && (
                                            <>
                                                <div className="mt-2">
                                                    <Label className="text-xs">Cash (optional)</Label>
                                                    <div className="relative mt-0.5">
                                                        ...
                                                        <Input
                                                            ...
                                                            step={0.01}
                                                            value={timeTradeCash}
                                                            onChange={(e) => setTimeTradeCash(e.target.value)}
                                                            placeholder="0"
                                                            className="h-8 pl-8"
                                                            onClick={(e) => e.stopPropagation()}
                                                        />
                                                ...
                                                {allowedStartTimes.length > 0 && (
                                                    <div className="mt-3" onClick={(e) => e.stopPropagation()}>
                                                        <Label className="text-xs">Preferred start times (in order)</Label>
                                                        <p className="text-[11px] text-muted-foreground mt-0.5">e.g. looking for 7am then 6am</p>
                                                        <div className="mt-1.5 flex flex-wrap gap-1.5">
                                                            {allowedStartTimes.map((t) => {
                                                                const raw = t.start_time;
                                                                const normalized = normalizeHhmm(raw);
                                                                const selected = preferredStartTimes.includes(normalized);
                                                                return (
                                                                    <button
                                                                        key={normalized}
                                                                        type="button"
                                                                        onClick={() => {
                                                                            if (selected) return;
                                                                            setPreferredStartTimes((prev) => [...prev, normalized]);
                                                                        }}
                                                                        className={cn(
                                                                            'rounded-md border px-2 py-1 text-xs font-medium transition-colors',
                                                                            selected ? 'border-primary bg-primary/10 text-primary cursor-default' : 'border-border hover:border-primary/50 hover:bg-muted/50'
                                                                        )}
                                                                        disabled={selected}
                                                                    >
                                                                        {formatTimeLabel(normalized)}
                                                                    </button>
                                                                );
                                                            })}
                                                        </div>
                                                        {preferredStartTimes.length > 0 && (
                                                            <ul className="mt-2 space-y-1">
                                                                {preferredStartTimes.map((t, i) => (
                                                                    <li key={`${t}-${i}`} className="flex items-center gap-1.5 rounded border border-border bg-muted/30 px-2 py-1 text-xs">
                                                                        <span className="text-muted-foreground w-4">{i + 1}.</span>
                                                                        <span className="flex-1">{formatTimeLabel(t)}</span>
                                                                        <div className="flex items-center gap-0.5">
                                                                            <button
                                                                                type="button"
                                                                                className="p-0.5 rounded hover:bg-muted"
                                                                                onClick={() => setPreferredStartTimes((prev) => {
                                                                                    if (i <= 0) return prev;
                                                                                    const next = [...prev];
                                                                            ...
                                                                            <button
                                                                                ...
                                                                                onClick={() => setPreferredStartTimes((prev) => prev.filter((_, j) => j !== i))}
                                                                                aria-label="Remove"
                                                                            >
                                                                                <X className="size-3.5" />
                                                                            </button>
                                ...
                                <input
                                    ...
                                    className={cn(
                                        'size-4 shrink-0 rounded-[4px] border border-input shadow-xs',
                                        'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2',
                                        postAsFF && 'border-primary bg-primary text-primary-foreground'
                                    )}
                                ...
                                <div className="flex-1">
                                    ...
                                    <p className="text-xs text-muted-foreground">Cover only part of this shift</p>
                                    {postAsFF && (
                                        <>
                                            <div className="mt-2">
                                                <Label className="text-xs">Minutes to cover</Label>
                                                <Input
                                                    type="number"
                                                    min={1}
                                                    max={600}
                                                    value={ffMinutes}
                                                    onChange={(e) => setFfMinutes(e.target.value)}
                                                    className="mt-0.5 h-8 w-24"
                                                    onClick={(e) => e.stopPropagation()}
                                                />
                                            </div>
                                            <div className="mt-2">
                                                <Label className="text-xs">Flight follower requested at</Label>
                                                <div className="mt-1 flex gap-3" onClick={(e) => e.stopPropagation()}>
                                                    <Label className="flex items-center gap-2 cursor-pointer">
                                                        <input
                                                            type="radio"
                                                            name="ff_at"
                                                            checked={ffAt === 'beginning'}
                                                            onChange={() => setFfAt('beginning')}
                                                            className="size-3.5"
                                                        />
                                                        <span className="text-sm">Beginning of shift</span>
                                                    </Label>
                                                    <Label className="flex items-center gap-2 cursor-pointer">
                                                        <input
                                                            type="radio"
                                                            name="ff_at"
                                                            checked={ffAt === 'end'}
                                                            onChange={() => setFfAt('end')}
                                                            className="size-3.5"
                                                        />
                                                        <span className="text-sm">End of shift</span>
                                                    </Label>
                                                </div>
                                            </div>
                                            <div className="mt-2">
                                                ...
                                                <div className="relative mt-0.5">
                                                    ...
                                                    <Input
                                                        ...
                                                        step={0.01}
                                                        value={ffCash}
                                                        onChange={(e) => setFfCash(e.target.value)}
                                                        placeholder="0"
                                                        className="h-8 pl-8"
                                                        onClick={(e) => e.stopPropagation()}
                                                    />
                        ...
                        {history.length > 0 && (
                            <div className="rounded-lg border border-border bg-muted/30 p-2">
                                ...
                                <ul className="mt-1.5 max-h-32 space-y-1.5 overflow-y-auto text-xs">
                                    {history.map((h, i) => (
                                        <li key={i} className="border-l-2 border-muted-foreground/30 pl-2">
                                            <span className="text-muted-foreground">
                                                {h.changed_at ? new Date(h.changed_at).toLocaleString() : ''}
                                                {h.post_type && ` · ${h.post_type.replace('_', ' ')}`}
                                            </span>
                                            <ul className="mt-0.5 space-y-0.5">
                                                {(h.changes || []).map((c, j) => {
                                                    const label = c.field === 'cash_amount' ? 'Cash amount' : c.field === 'flight_follow_minutes' ? 'Minutes to cover' : c.field === 'flight_follow_at' ? 'Flight follower at' : c.field === 'notes' ? 'Notes' : c.field;
                                                    const oldStr = c.old != null && c.old !== '' ? (c.field === 'cash_amount' ? `$${c.old}` : String(c.old)) : '(empty)';
                                                    const newStr = c.new != null && c.new !== '' ? (c.field === 'cash_amount' ? `$${c.new}` : String(c.new)) : '(empty)';
                                                    return (
                    ...
                    <DialogFooter className="flex flex-col gap-3 pt-4 sm:flex-row sm:flex-wrap sm:items-center">
                        {hasExisting && (
                            <Button
                                ...
                                onClick={() => setShowConfirmRemovePost(true)}
                            >
        ...
        <Dialog open={showConfirmRemovePost} onOpenChange={setShowConfirmRemovePost}>
            <DialogContent className="sm:max-w-md" aria-describedby={undefined}>
                <DialogHeader>
                    <DialogTitle>Remove post?</DialogTitle>
                </DialogHeader>
                <p className="text-sm text-muted-foreground">
                    This will remove all postings for this shift. The shift will stay on your schedule; you just won’t be offering it for trade, giveaway, or flight following anymore.
                </p>
                <DialogFooter className="gap-2 pt-4">
                    <Button type="button" variant="outline" onClick={() => setShowConfirmRemovePost(false)}>
                        Cancel
                    </Button>
                    <Button
                        type="button"
