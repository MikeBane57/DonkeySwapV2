...
test('group ranked mode sorts cross-bucket lines in the same group by holidays when desk is first', function () {
    ...
    $import = app(BidLineCsvImportService::class)->importFromPath(
        $path,
        'group-cross-bucket-holiday.csv',
        $user->id,
        $bidYear,
        null,
        'Group cross-bucket holiday import',
    )['import'];

    @unlink($path);

    $lowHolidayLine = BidLine::query()->where('bid_import_id', $import->id)->where('line_num', '551')->firstOrFail();
    $highHolidayLine = BidLine::query()->where('bid_import_id', $import->id)->where('line_num', '552')->firstOrFail();

    $scenario = BidScenario::create([
        'user_id' => $user->id,
        'bid_import_id' => $import->id,
        'name' => 'Group ranked cross-bucket holidays',
        'vacation_bank' => 10,
        'weights' => [
            'holiday' => 0,
            'personal' => 0,
    ...
    $scores = app(ScenarioScoreService::class)->scoreLines(
        $scenario,
        [$lowHolidayLine->id, $highHolidayLine->id],
    );

    $byId = collect($scores)->keyBy('bid_line_id');

    expect($byId[$lowHolidayLine->id]['tier_ranks']['desk'])->toBe($byId[$highHolidayLine->id]['tier_ranks']['desk']);
    expect($byId[$lowHolidayLine->id]['tier_ranks']['desk_order'])
        ->toBeLessThan($byId[$highHolidayLine->id]['tier_ranks']['desk_order']);
    expect($byId[$highHolidayLine->id]['sort_scores']['holiday'])
        ->toBeGreaterThan($byId[$lowHolidayLine->id]['sort_scores']['holiday']);
    expect($scores[0]['bid_line_id'])->toBe($highHolidayLine->id);
});

test('group ranked mode applies desk list order only among lines in the same bucket', function () {
    config(['features.bid_tools' => true]);
    ...
    $path = writeSameGroupHolidayScoreCsv($bidYear, [
        '551' => [$firstHoliday],
        '552' => [$firstHoliday, $secondHoliday],
    ]);

    $import = app(BidLineCsvImportService::class)->importFromPath(
        $path,
        'group-same-bucket-desk.csv',
        $user->id,
        $bidYear,
        null,
        'Group same-bucket desk import',
    )['import'];

    @unlink($path);

    $oneHolidayLine = BidLine::query()->where('bid_import_id', $import->id)->where('line_num', '551')->firstOrFail();
    $twoHolidayLine = BidLine::query()->where('bid_import_id', $import->id)->where('line_num', '552')->firstOrFail();

    $scenario = BidScenario::create([
        'user_id' => $user->id,
        'bid_import_id' => $import->id,
        'name' => 'Same bucket desk order',
        'vacation_bank' => 10,
        'weights' => [
            'holiday' => 0,
            'personal' => 0,
    ...
    $scores = $scoreService->scoreLines(
        $scenario,
        [$oneHolidayLine->id, $twoHolidayLine->id],
    );

    $byId = collect($scores)->keyBy('bid_line_id');

    expect($byId[$oneHolidayLine->id]['tier_ranks']['desk_order'])
        ->toBe($byId[$twoHolidayLine->id]['tier_ranks']['desk_order']);
    expect($byId[$twoHolidayLine->id]['sort_scores']['holiday'])
        ->toBeGreaterThan($byId[$oneHolidayLine->id]['sort_scores']['holiday']);
    expect($scores[0]['bid_line_id'])->toBe($twoHolidayLine->id);
});
...
test('group ranked mode partitions by synced visual desk groups', function () {
    ...
    expect($byId[$dsLine->id]['tier_ranks']['desk_group'])
        ->toBe($byId[$dgLine->id]['tier_ranks']['desk_group']);
});
